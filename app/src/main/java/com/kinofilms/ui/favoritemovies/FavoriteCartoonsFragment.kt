...
import com.kinofilms.databinding.FragmentFavoriteCartoonsBinding
import com.kinofilms.models.Movie
import com.kinofilms.ui.favoritemovies.adapter.FavoriteAdapter
import dagger.hilt.android.AndroidEntryPoint

@AndroidEntryPoint
class FavoriteCartoonsFragment : Fragment() {

    private lateinit var binding: FragmentFavoriteCartoonsBinding
    ...
    override fun onCreateView(
        ...
        binding = FragmentFavoriteCartoonsBinding.inflate(inflater, container, false)
        return binding.root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        with(binding.toolbarCustom) {
            ...
            toolbarTitle.text = getString(R.string.favorites_cartoons)
        ...
        viewModel.listFavorite.observe(viewLifecycleOwner) {
            initListFavorite(it.filter { movie ->
                movie.type == getString(R.string.type_cartoon)
            })
        }
    ...
    private fun initListFavorite(list: List<Movie>) {
        binding.listFavoriteCartoons.run {
            if (adapter == null) {
                adapter = FavoriteAdapter {
                    findNavController().navigate(
                        FavoriteCartoonsFragmentDirections.actionFavoriteCartoonsFragmentToMovieFragment(
                            it.id.toString()
                        )
                    )
                }
